 ...
 *
 * @class
 * @param {MatomoOptions} userOptions - User configuration options for Matomo tracking.
...
class MatomoTracker {
  ...
  constructor(userOptions: MatomoOptions) {
   ...
   *
   * @param {MatomoOptions} options - Initialization options.
   ...
   * @param {UserInfo} [options.userInfo={}] - Optional data for tracking different user info.
   * @returns {Promise<Response>} A Promise that resolves when the tracking is complete.
   *
   ...
   * @param {UserInfo} [options.userInfo={}] - Optional data used for tracking different user information. See {@link https://developer.matomo.org/api-reference/tracking-api#optional-user-info} for details.
   * @throws {Error} Throws an error if the 'name' parameter is not provided.
   * @returns {Promise<Response>} A Promise that resolves when the screen view tracking is complete.
   *
   * @example
   ...
   * @param {UserInfo} [options.userInfo={}] - Optional data used for tracking different user information. See {@link https://developer.matomo.org/api-reference/tracking-api#optional-user-info} for details.
   * @throws {Error} Throws an error if the 'name' parameter is not provided.
   * @returns {Promise<Response>} A Promise that resolves when the action tracking is complete.
   *
   * @example
   ...
   * @param {number} [options.value] - The event value. Must be a float or integer value (numeric), not a string.
   ...
   * @returns {Promise<Response>} A Promise that resolves when the event tracking is complete.
   *
   * @example
   ...
   * @returns {Promise<Response>} A Promise that resolves when the content tracking is complete.
   *
   * @example
   ...
   * @returns {Promise<Response>} A Promise that resolves when the site search tracking is complete.
   *
   * @example
   ...
   * @returns {Promise<Response>} A Promise that resolves when the link click tracking is complete.
   *
   * @example
   ...
   * @returns {Promise<Response>} A Promise that resolves when the download tracking is complete.
   *
   * @example
   ...
   *
   * @param {TrackingData} data - The tracking data.
   * @returns {Promise<Response | Error>} A Promise that resolves when the tracking data is sent.
   */
  private track(data: TrackingData): Promise<Response | Error> {
    if (this.disabled) return Promise.resolve() as unknown as Promise<Response | Error>
    if (!data) return Promise.resolve() as unknown as Promise<Response | Error>
    ...
    const params: Record<string, string> = {
      ...
      ...Object.fromEntries(Object.entries(this.userInfo).map(([key, value]) => [key, String(value)]))
    }
    ...
    const headers: Record<string, string> = {
      ...
      'Accept-Language': typeof lang === 'string' ? lang : 'en',
    ...
    // console.log('Matomo tracking data:', this.trackerUrl, fetchObj)
    if (!this.trackerUrl) {
      return Promise.reject(new Error('Tracker URL is not initialized'))
    }
    ...
    return fetch(this.trackerUrl, fetchObj)
      ...
      .catch(error => {
        if (this.log) {
          // eslint-disable-next-line no-console
          console.log('Matomo tracking is not sent:', this.trackerUrl, fetchObj)
        }
